...
<style>
    ...
    .carousel-container {
        ...
        padding: 5px 0;
        z-index: 5;
        margin: 0 -5%; /* EBEVEYN PADDING'İNİ SIFIRLAMAK İÇİN EKLENDİ */
    }
    ...
    .swiper-container {
        ...
        padding: 0 5%; /* YENİ PADDING BURAYA EKLENDİ */
        box-sizing: border-box; /* HESAPLAMA İÇİN EKLENDİ */
    }
...
.action-btn.play-btn .fas {
    ...
    .carousel-container::after {
        ...
        top: 0;
        bottom: 0;
        width: 8%; /* GENİŞLİK ARTIRILDI */
        z-index: 9; /* Okların üzerinde ama hover kartın altında */
        pointer-events: none; /* Tıklamayı engellememesi için */
    }

    .carousel-container::before {
        left: 0;
        background: linear-gradient(to right, rgb(7, 7, 7) 0%, rgba(7, 7, 7, 0) 100%);
    }

    .carousel-container::after {
        right: 0;
        background: linear-gradient(to left, rgb(7, 7, 7) 0%, rgba(7, 7, 7, 0) 100%);
    }
    ...
    @media (max-width: 768px) {
        ...
        .carousel-container {
            margin: 0 -3%; /* Mobildeki padding'e göre ayarla */
        }
        .swiper-container {
            padding: 0 3%; /* Mobildeki padding'e göre ayarla */
        }
        
        .carousel-container::before,
        .carousel-container::after {
            width: 5%; /* Mobil genişlik artırıldı */
        }
...
<div class="hover-actions">
    <?php
        // --- YENİ İZLEME LİSTESİ MANTIĞI ---
        // Döngüdeki mevcut öğeyi ($item, $heroContent vb.) alıyoruz
        $currentItem = $item ?? $heroContent ?? null;
        
        // Gerekli değişkenleri ayarlıyoruz
        $isTvShow = isset($currentItem['first_air_date']) || isset($currentItem['name']);
        $type = $isTvShow ? 'tv_show' : 'movie';
        $id = $currentItem['id'];

        // Öğenin izleme listesinde olup olmadığını kontrol ediyoruz
        $isInWatchlist = false;
        ...
        $watchUrl = $isTvShow ? ($currentItem['watch_url'] ?? '#') : ('/movie/' . ($currentItem['slug'] ?? '') . '/watch');
        $detailUrl = ($isTvShow ? '/tv-show' : '/movie/') . ($currentItem['slug'] ?? '');
    ?>
